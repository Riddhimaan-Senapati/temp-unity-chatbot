    ...
    try:
        ...
        if file_extension in ['txt', 'md', 'markdown']:
            ...
            try:
                ...
                logger.warning(f"Could not decode file {key} as UTF-8, treating as binary")
                return None
...
def lambda_handler(event, context):
    ...
    try:
        ...
        for record in event.get('Records', []):
            ...
            logger.info(f"Processing {event_name} event for {key} in bucket {bucket_name}")
            ...
            logger.info(f"Processing {key} - will create/update metadata file: {metadata_key}")
            ...
            try:
                llm_response = invoke_model_sync(bedrock_runtime, prompt, max_tokens=300)
