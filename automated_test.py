...
from chatbot_helper import (initialize_bedrock_client, initialize_knowledge_base_retriever,
query_without_system_prompt, query_with_system_prompt) 
...
def test_model_on_database(model_name):
    ...
    for thread_idx, thread in enumerate(all_threads):
        ...
        for msg_idx, question in enumerate(thread["messages"]):
            print(f"  Question {msg_idx + 1}/{len(thread['messages'])}: {question[:25]}...")
            ...
            thread_metrics["total_response_time"] += metrics.get('response_time_seconds', 0)
        ...
        if thread_metrics["total_queries"] > 0:
            thread_metrics["average_response_time"] = round(
                thread_metrics["total_response_time"] / thread_metrics["total_queries"], 3
            )
            if thread_metrics["total_response_time"] > 0:
                total_tokens = thread_metrics["total_input_tokens"] + thread_metrics["total_output_tokens"]
...
def database_json_to_markdown(json_file):
    ...
    markdown += f"- **Total Response Time:** {summary['total_response_time']:.3f} seconds\n"
    markdown += f"- **Average Response Time:** {summary['average_response_time']:.3f} seconds\n"
    ...
    markdown += f"- **Average Tokens/Second:** {summary['average_tokens_per_second']:.2f}\n"
    ...
    for thread_name, thread_data in results['threads'].items():
        ...
        for message in thread_data['messages']:
            ...
            markdown += f"- Tokens/Second: {metrics.get('tokens_per_second', 'N/A'):.2f}\n"
...
def compare_models(thread_name):
    ...
    print(f"Processing {len(MODELS)} models with {len(thread['messages'])} questions each...")
    ...
    for model_idx, (model_name, model_id) in enumerate(MODELS.items()):
        ...
        for msg_idx, question in enumerate(thread["messages"]):
            print(f"  Question {msg_idx + 1}/{len(thread['messages'])}: {question[:50]}...")
            ...
            try:
                ...
                model_metrics["total_response_time"] += metrics.get('response_time_seconds', 0)
        ...
        if model_metrics["total_queries"] > 0:
            ...
            if model_metrics["total_response_time"] > 0:
                total_tokens = model_metrics["total_input_tokens"] + model_metrics["total_output_tokens"]
        ...
        results["summary_metrics"]["total_queries_all_models"] += model_metrics["total_queries"]
        
        # Update best performance tracking
        best_perf = results["summary_metrics"]["best_performance"]
        
        # Fastest model
        if model_metrics["average_response_time"] > 0 and model_metrics["average_response_time"] < best_perf["fastest_avg_time"]:
        ...
        if model_metrics["average_tokens_per_second"] > best_perf["highest_tokens_per_second"]:
            best_perf["most_tokens_per_second"] = model_name
            best_perf["highest_tokens_per_second"] = model_metrics["average_tokens_per_second"]
    ...
    print(f"Questions per model: {results['summary_metrics']['total_queries_per_model']}")
    print(f"Fastest model: {best_perf['fastest_model']} ({best_perf['fastest_avg_time']:.3f}s avg)")
    print(f"Cheapest model: {best_perf['cheapest_model']} (${best_perf['lowest_cost']:.6f})")
    print(f"Highest tokens/sec: {best_perf['most_tokens_per_second']} ({best_perf['highest_tokens_per_second']:.2f})")
...
def compare_models_json_to_markdown(json_file):
    ...
    markdown += f"- **Total Queries (All Models):** {summary['total_queries_all_models']}\n\n"
    ...
    for model_name, model_data in results['models'].items():
        ...
        markdown += f"- Total Output Tokens: {model_metrics['total_output_tokens']:,}\n\n"
...
def test_model_with_without_system_prompt(model_name, thread_name):
    ...
    print(f"Testing both WITH and WITHOUT system prompt")
    ...
    for msg_idx, question in enumerate(thread["messages"]):
        ...
        try:
            ...
            results["configurations"]["with_system_prompt"]["messages"].append(message_result_with)
            
            # Update metrics for with system prompt
            with_metrics = results["configurations"]["with_system_prompt"]["metrics"]
            with_metrics["total_response_time"] += metrics_with.get('response_time_seconds', 0)
            ...
            results["configurations"]["with_system_prompt"]["messages"].append(error_result_with)
        
        # Test WITHOUT system prompt
        print("  Testing without system prompt...")
        try:
            response_without, sources_without, metrics_without = query_without_system_prompt(
                client, retriever, question, model_id
            )
            ...
            results["configurations"]["without_system_prompt"]["messages"].append(message_result_without)
            
            # Update metrics for without system prompt
            without_metrics = results["configurations"]["without_system_prompt"]["metrics"]
            without_metrics["total_response_time"] += metrics_without.get('response_time_seconds', 0)
            without_metrics["total_input_tokens"] += metrics_without.get('input_tokens', 0)
            without_metrics["total_output_tokens"] += metrics_without.get('output_tokens', 0)
            ...
            results["configurations"]["without_system_prompt"]["messages"].append(error_result_without)
    ...
    for config_name, config_data in results["configurations"].items():
        ...
        if config_metrics["total_queries"] > 0:
            config_metrics["average_response_time"] = round(
                config_metrics["total_response_time"] / config_metrics["total_queries"], 3
            )
            if config_metrics["total_response_time"] > 0:
                total_tokens = config_metrics["total_input_tokens"] + config_metrics["total_output_tokens"]
    ...
    comparison["response_time_difference"] = round(
        with_metrics["average_response_time"] - without_metrics["average_response_time"], 3
    )
    ...
    comparison["tokens_per_second_difference"] = round(
        with_metrics["average_tokens_per_second"] - without_metrics["average_tokens_per_second"], 2
    )
    
    # Determine which configuration is better
    comparison["faster_configuration"] = "with_system_prompt" if with_metrics["average_response_time"] < without_metrics["average_response_time"] else "without_system_prompt"
    comparison["cheaper_configuration"] = "with_system_prompt" if with_metrics["total_cost"] < without_metrics["total_cost"] else "without_system_prompt"
    comparison["more_efficient_configuration"] = "with_system_prompt" if with_metrics["average_tokens_per_second"] > without_metrics["average_tokens_per_second"] else "without_system_prompt"
    ...
    print(f"Model: {model_name}")
    print(f"Thread: {thread_name}")
    print(f"Faster configuration: {comparison['faster_configuration']} (difference: {comparison['response_time_difference']:.3f}s)")
    print(f"Cheaper configuration: {comparison['cheaper_configuration']} (difference: ${comparison['cost_difference']:.6f})")
    print(f"More efficient configuration: {comparison['more_efficient_configuration']} (difference: {comparison['tokens_per_second_difference']:.2f} tokens/s)")
...
def system_prompt_comparison_json_to_markdown(json_file):
    ...
    for config_name, config_data in results['configurations'].items():
        ...
        config_metrics = config_data['metrics']
        markdown += f"**Configuration Summary:**\n"
        ...
        markdown += f"- Total Output Tokens: {config_metrics['total_output_tokens']:,}\n\n"
